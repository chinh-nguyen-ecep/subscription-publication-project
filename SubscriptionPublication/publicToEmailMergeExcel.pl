...
sub main{
	...
	$i=0;
	my $excelFileMerged=$export_dir."/".$subscription_name.".".$time_tmp.".xls";
	my $excelCount=0;
	foreach my $rows(@listFileToUpload){
		...
		my ($base_name, $dir, $suffix) = fileparse($rows->[1], '.xls');		
		if($suffix eq ".xls"){
			$excelCount++;
			print "Merge Excel file: ".$rows->[1]."\n";
			my @cmd=`cd $folder_content_transfer_script && java -jar mergeExcel.jar \"$excelFileMerged\"  \"$export_dir/$rows->[1]\" 2>log/$pid`;
	...
	if($excelCount>0){
		$file_input=$file_input." \"".$excelFileMerged."\"";
	}
	...
	my $email_content="\"$subscription_desc <p/>This email is generated by Auto Subscription/publication system. Please do not reply to this email address.<p/>Regards,<br/>Dataman.\"";
	...
	#remove merge excel file
	system("rm -rf \"".$excelFileMerged."\"");
